...
export function MiseDinnerPlaylist({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Vertically Stacked Days 1-3 (or 1-4) */}
      <div className="space-y-5">
        {currentSlots.map((slot) => {
          const { day, locked, recipe } = slot;
          return (
            <div
              key={day}
              className={`bg-[#140f20] rounded-3xl border ${
                locked ? 'border-amber-500/40 ring-1 ring-amber-500/20' : 'border-purple-900/30'
              } p-5 sm:p-7 shadow-2xl space-y-4 transition-all`}
            >
              {/* Day Header with Badges & Action Buttons */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/5">
                ...
                >
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded font-mono shadow-xs">
                      Day {day} Dinner
                    </span>
                    ...
                    {recipe.sourceType === 'cookbook' && (
                      <span className="bg-amber-500/20 text-amber-300 border border-amber-500/30 text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1">
                        <BookOpen className="w-3 h-3" /> {recipe.cookbookTitle}
                      </span>
                    )}
                  </div>

                  <h3 className="text-xl sm:text-2xl font-serif font-bold text-white leading-tight group-hover:text-purple-200 transition-colors flex items-center gap-2 pt-1">
                ...
                <div className="flex items-center gap-2 shrink-0 flex-wrap">
                  <button
                    type="button"
                    onClick={() => handleMarkCooked(day, recipe)}
                    disabled={loggingCookedDay === day}
                    className="px-3.5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-md transition-all active:scale-95 disabled:opacity-50"
                  >
                    <Check className="w-3.5 h-3.5 stroke-[3]" />
                    <span>{loggingCookedDay === day ? 'Logging...' : 'Cooked Tonight'}</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => handleToggleLock(day)}
                    title={locked ? 'Unlock meal for shuffle' : 'Lock meal to preserve during shuffle'}
                    className={`p-2.5 rounded-xl border text-xs font-bold flex items-center gap-1.5 cursor-pointer transition-all ${
                      locked
                        ? 'bg-amber-950/60 border-amber-500 text-amber-300 shadow-xs'
                  ...
                  >
                    ...
                    <span>{locked ? 'Pinned' : 'Pin'}</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => handleSwapSlot(day)}
                    disabled={swappingDay === day}
                  ...
                  >
                    <RotateCcw className={`w-3.5 h-3.5 ${swappingDay === day ? 'animate-spin' : ''}`} />
                    <span>Swap</span>
                  </button>
                </div>
              </div>
              ...
              {/* Quick Specs */}
              <div className="flex items-center gap-4 text-xs text-purple-200/70 flex-wrap">
                {recipe.cookTime && (
              ...
              <div className="space-y-2 pt-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                  {recipe.ingredients?.slice(0, 6).map((ing: any, i: number) => (
                    <div
                      key={i}
                      className="p-2.5 rounded-xl bg-white/[0.03] border border-white/5 text-xs text-purple-100 flex items-center justify-between truncate"
                    >
                      <span className="truncate">{ing.name}</span>
                      {ing.amount && (
                        <span className="text-purple-300/60 font-mono text-[10px] ml-1 shrink-0">
                          {ing.amount}
                        </span>
                      )}
                    </div>
                  ...
                  {recipe.ingredients?.length > 6 && (
                    <button
                      type="button"
                      onClick={() => setSelectedRecipeDetail(recipe)}
                      className="p-2.5 rounded-xl bg-purple-950/40 border border-purple-800/30 text-xs text-purple-300 hover:text-white font-medium flex items-center justify-center cursor-pointer"
                    >
      ...
      {selectedRecipeDetail && (
        <MiseRecipeDetailModal
          ...
          onMarkCooked={(r) => {
            const matchSlot = currentSlots.find((s) => s.recipe?.id === r.id);
            handleMarkCooked(matchSlot ? matchSlot.day : 1, r);
          }}
